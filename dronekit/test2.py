...
time.sleep(2)
print("BEGINNING MODE IS:", vehicle.mode.name)
...
while True:
    ...
    if nextwaypoint != 0 and distance < 70 and ascend:
      
        currentwaypoint = nextwaypoint
        lastmissionitem = vehicle.commands[nextwaypoint-1]
        lat = lastmissionitem.x
        lon = lastmissionitem.y
        alt = lastmissionitem.z
        targetWaypointLocation = LocationGlobalRelative(lat,lon,alt)
        ...
        while True:
            print(vehicle.mode.name)
            distancetopoint = get_distance_metres(vehicle.location.global_frame, targetWaypointLocation)
            print("Altitude:", vehicle.location.global_relative_frame.alt, "  Distance:", distancetopoint)   
            #print(" Latitude: ", vehicle.location.global_relative_frame.lat)   
            #print(" Longitude: ", vehicle.location.global_relative_frame.lon)      
            if vehicle.location.global_relative_frame.alt>=newalt*0.95: #Trigger just below target alt.
                print("Reached target altitude")
                arrived = True
                break
            time.sleep(1)
        ...
        vehicle.commands.upload()
        vehicle.commands.next = currentwaypoint
        newalt+=10
        ascend = False
    
    if arrived:
        vehicle.mode = VehicleMode("AUTO")
        print("VEHICLE MODE SHOULD BE AUTO:", vehicle.mode.name)
